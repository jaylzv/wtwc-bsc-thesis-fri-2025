...
import Config, { ACTION_ASSIST, ACTION_DISABLE_AUTOCONSENT } from '../../../store/config.js';
...
const __vite_glob_0_5 = {
  ...
  config: store(Config),
  updatedAt: ({ options }) => store.ready(options) && options.filtersUpdatedAt && formatDate(options.filtersUpdatedAt),
  visible: false,
  render: ({ visible, counter, updatedAt, config }) => html`
    ...
    ([name, d]) => html`<ui-text color="gray-600">
                            ${name} (${d.actions.join(", ")})
                          </ui-text>`
  ) || "none"}
                  </div>
                ...
                <div>
                  ...
                  <ui-text color="gray-600">
                    ${Object.entries(config.flags).filter(([, f]) => f.enabled).map(([name]) => name).join(" ") || "none"}
                  </ui-text>
                </div>
                <div layout="row gap">
                  <ui-button
